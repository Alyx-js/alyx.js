...
class WebhookClient {
	...
	*/
	sendMessage(msg) {
		...
		superagent
		 .post(`https://discord.com/api/v${Constants.API_VERSION}/webhooks/${this.ID}/${this.TOKEN}`)
		 .send({content: msg})
		 .then(resolve)
	...
	*/
	destroy() {
		superagent
		 .delete(`https://discord.com/api/v${Constants.API_VERSION}/webhooks/${this.ID}/${this.TOKEN}`)
	...
	*/
	setName(name) {
		return new Promise((resolve, reject) => {
		superagent
		 .patch(`https://discord.com/api/v${Constants.API_VERSION}/webhooks/${this.ID}/${this.TOKEN}`)
		 .send({name: name})
